...
- 動作可在網頁設定為 `mute` / `kick` / `ban` / `add_role`（加指定身分組）
- 可切換為「只處理未認證成員」
...
> 若 `git clone` 顯示 `destination path ... already exists`，表示舊資料夾已存在；請先用 `Test-Path` 檢查是否含 `examples/discord-voice-switch-guard`，或改用新資料夾名稱重新 clone。
